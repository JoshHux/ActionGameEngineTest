...
{
	public delegate Fix64 SortKeyFunc(object shape);
...
#warning "CAS"
	...
	public class BroadPhase
	{
		...
		public Fix64[] _querySortKeys = new Fix64[Settings.MaxProxies];
		public int _queryResultCount;

		public AABB _worldAABB;
		public FVec2 _quantizationFactor;
		...
		public BroadPhase(AABB worldAABB, PairCallback callback)
		{
			...
			FVec2 d = worldAABB.UpperBound - worldAABB.LowerBound;
			_quantizationFactor.X = (Fix64)BROADPHASE_MAX / d.X;
			_quantizationFactor.Y = (Fix64)BROADPHASE_MAX / d.Y;
		...
		{
			FVec2 d = Common.Math.Max(aabb.LowerBound - _worldAABB.UpperBound, _worldAABB.LowerBound - aabb.UpperBound);
			return Common.Math.Max(d.X, d.Y) < Fix64.Zero;
		}
...
#warning "Check this"
		...
		int QuerySegment(Segment segment, object[] userData, int maxCount, SortKeyFunc sortKey)
		{
			...
			Fix64 dx = (segment.P2.X - segment.P1.X) * _quantizationFactor.X;
			Fix64 dy = (segment.P2.Y - segment.P1.Y) * _quantizationFactor.Y;
			...
			Fix64 p1x = (segment.P1.X - _worldAABB.LowerBound.X) * _quantizationFactor.X;
			Fix64 p1y = (segment.P1.Y - _worldAABB.LowerBound.Y) * _quantizationFactor.Y;
#if ALLOWUNSAFE
			...
			// TODO_ERIN implement fast Fix64 to ushort conversion.
			...
			if (sortKey != null)
			{
				...
				{
					Fix64 a = _querySortKeys[i];
					Fix64 b = _querySortKeys[i + 1];
					if ((a < 0) ? (b >= 0) : (a > b && b >= 0))
					{
		...
		private void ComputeBounds(out ushort[] lowerValues, out ushort[] upperValues, AABB aabb)
		{
			...
			FVec2 minVertex = Common.Math.Clamp(aabb.LowerBound, _worldAABB.LowerBound, _worldAABB.UpperBound);
			FVec2 maxVertex = Common.Math.Clamp(aabb.UpperBound, _worldAABB.LowerBound, _worldAABB.UpperBound);

			// Bump lower bounds downs and upper bounds up. This ensures correct sorting of
			// lower/upper bounds that would have equal values.
			// TODO_ERIN implement fast Fix64 to uint16 conversion.
		...
		{
			Fix64 key = sortKey(proxy.UserData);
			//Filter proxies on positive keys
			if (key < 0)
				return;
			//Merge the new key into the sorted list.
			//Fix6432* p = std::lower_bound(m_querySortKeys,m_querySortKeys+m_queryResultCount,key);
			fixed (Fix64* querySortKeysPtr = _querySortKeys)
			{
				Fix64* p = querySortKeysPtr;
		...
		public void AddProxyResult(ushort proxyId, Proxy proxy, int maxCount, SortKeyFunc sortKey)
		{
			...
			//Merge the new key into the sorted list.
			//Fix6432* p = std::lower_bound(m_querySortKeys,m_querySortKeys+m_queryResultCount,key);
			Fix64[] querySortKeysPtr = _querySortKeys;

			int ip = 0;
			Fix64 p = querySortKeysPtr[ip];
