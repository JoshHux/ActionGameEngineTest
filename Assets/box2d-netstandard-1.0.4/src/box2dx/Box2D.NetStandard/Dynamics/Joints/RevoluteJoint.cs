...
namespace Box2DX.Dynamics
{
	...
	/// </summary>
	public class RevoluteBJointDef : BJointDef
	{
		public RevoluteBJointDef()
		{
			Type = BJointType.RevoluteBJoint;
		...
		/// anchor.
		/// </summary>
		public void Initialize(Body body1, Body body2, FVec2 anchor)
		{
			Body1 = body1;
			Body2 = body2;
	...
	/// </summary>
	public class RevoluteBJoint : BJoint
	{
		public FVec2 _localAnchor1;	// relative
		...
		{
			FVec2 P = new FVec2(_impulse.X, _impulse.Y);
			return inv_dt * P;
		}
		...
		public RevoluteBJoint(RevoluteBJointDef def)
			: base(def)
		{
		...
		internal override void InitVelocityConstraints(TimeStep step)
		{
			...
			if (_enableLimit)
			{
				...
				if (Box2DXMath.Abs(_upperAngle - _lowerAngle) < (Fix64)2.0f * Settings.AngularSlop)
				{
			...
			if (step.WarmStarting)
			{
				...
				b1._angularVelocity -= i1 * (FVec2.Cross(r1, P) + _motorImpulse + _impulse.Z);

				b2._linearVelocity += m2 * P;
				b2._angularVelocity += i2 * (FVec2.Cross(r2, P) + _motorImpulse + _impulse.Z);
			}
			else
			{
				_impulse.SetZero();
		...
		internal override void SolveVelocityConstraints(TimeStep step)
		{
			...
			if (_enableLimit && _limitState != LimitState.InactiveLimit)
			{
				...
				FVec2 Cdot1 = v2 + FVec2.Cross(w2, r2) - v1 - FVec2.Cross(w1, r1);
				Fix64 Cdot2 = w2 - w1;
				FVec3 Cdot = new FVec3(Cdot1.X, Cdot1.Y, Cdot2);

				FVec3 impulse = _mass.Solve33(-Cdot);
				...
				else if (_limitState == LimitState.AtLowerLimit)
				{
					...
					{
						FVec2 reduced = _mass.Solve22(-Cdot1);
						impulse.X = reduced.X;
						impulse.Y = reduced.Y;
				...
				FVec2 P = new FVec2(impulse.X, impulse.Y);

				v1 -= m1 * P;
				w1 -= i1 * (FVec2.Cross(r1, P) + impulse.Z);

				v2 += m2 * P;
				w2 += i2 * (FVec2.Cross(r2, P) + impulse.Z);
			}
			else
			{
				FVec2 r1 = Box2DXMath.Mul(b1.GetXForm().R, _localAnchor1 - b1.GetLocalCenter());
				...
				// Solve point-to-point constraint
				FVec2 Cdot = v2 + FVec2.Cross(w2, r2) - v1 - FVec2.Cross(w1, r1);
				FVec2 impulse = _mass.Solve22(-Cdot);

				_impulse.X += impulse.X;
				_impulse.Y += impulse.Y;

				v1 -= m1 * impulse;
				w1 -= i1 * FVec2.Cross(r1, impulse);

				v2 += m2 * impulse;
				w2 += i2 * FVec2.Cross(r2, impulse);
			}
		...
		internal override bool SolvePositionConstraints(Fix64 baumgarte)
		{
			...
			// Solve point-to-point constraint.
			{
				...
				FVec2 C = b2._sweep.C + r2 - b1._sweep.C - r1;
				...
				Fix64 k_allowedStretch = (Fix64)10.0f * Settings.LinearSlop;
				if (C.LengthSquared() > k_allowedStretch * k_allowedStretch)
				{
					// Use a particle solution (no rotation).
					FVec2 u = C; u.Normalize();
					Fix64 k = invMass1 + invMass2;
					Box2DXDebug.Assert(k > Settings.FLT_EPSILON);
					Fix64 m = Fix64.One / k;
					FVec2 impulse = m * (-C);
					Fix64 k_beta = (Fix64)0.5f;
				...
				Mat22 K = K1 + K2 + K3;
				FVec2 impulse_ = K.Solve(-C);

				b1._sweep.C -= b1._invMass * impulse_;
				b1._sweep.A -= b1._invI * FVec2.Cross(r1, impulse_);

				b2._sweep.C += b2._invMass * impulse_;
				b2._sweep.A += b2._invI * FVec2.Cross(r2, impulse_);
