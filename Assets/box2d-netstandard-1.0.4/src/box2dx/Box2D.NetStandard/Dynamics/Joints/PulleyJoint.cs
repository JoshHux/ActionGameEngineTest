...
namespace Box2DX.Dynamics
{
	...
	/// and a pulley ratio.
	/// </summary>
	public class PulleyBJointDef : BJointDef
	{
		public PulleyBJointDef()
		{
			Type = BJointType.PulleyBJoint;
		...
		public void Initialize(Body body1, Body body2,
						FVec2 groundAnchor1, FVec2 groundAnchor2,
						FVec2 anchor1, FVec2 anchor2,
						Fix64 ratio)
		{
			Body1 = body1;
			Body2 = body2;
			...
			FVec2 d1 = anchor1 - groundAnchor1;
			Length1 = d1.Length();
			FVec2 d2 = anchor2 - groundAnchor2;
			Length2 = d2.Length();
			Ratio = ratio;
			Box2DXDebug.Assert(ratio > Settings.FLT_EPSILON);
			Fix64 C = Length1 + ratio * Length2;
			MaxLength1 = C - ratio * PulleyBJoint.MinPulleyLength;
			MaxLength2 = (C - PulleyBJoint.MinPulleyLength) / ratio;
		}

		/// <summary>
		/// The first ground anchor in world coordinates. This point never moves.
		/// </summary>
		public FVec2 GroundAnchor1;

		/// <summary>
		/// The second ground anchor in world coordinates. This point never moves.
		/// </summary>
		public FVec2 GroundAnchor2;
	...
	/// </summary>
	public class PulleyBJoint : BJoint
	{
		public static readonly Fix64 MinPulleyLength = (Fix64)2.0f;
		...
		public Fix64 Length1
		{
			get
			{
				FVec2 p = _body1.GetWorldPoint(_localAnchor1);
				FVec2 s = _ground.GetXForm().Position + _groundAnchor1;
				FVec2 d = p - s;
				return d.Length();
			}
		}
		...
		public Fix64 Length2
		{
			get
			{
				FVec2 p = _body2.GetWorldPoint(_localAnchor2);
				FVec2 s = _ground.GetXForm().Position + _groundAnchor2;
				FVec2 d = p - s;
				return d.Length();
			}
		}
		...
		public PulleyBJoint(PulleyBJointDef def)
			: base(def)
		{
			...
			_maxLength1 = Common.Math.Min(def.MaxLength1, _constant - _ratio * PulleyBJoint.MinPulleyLength);
			_maxLength2 = Common.Math.Min(def.MaxLength2, (_constant - PulleyBJoint.MinPulleyLength) / _ratio);
		...
		internal override void InitVelocityConstraints(TimeStep step)
		{
			...
			FVec2 r1 = Box2DXMath.Mul(b1.GetXForm().R, _localAnchor1 - b1.GetLocalCenter());
			FVec2 r2 = Box2DXMath.Mul(b2.GetXForm().R, _localAnchor2 - b2.GetLocalCenter());

			FVec2 p1 = b1._sweep.C + r1;
			FVec2 p2 = b2._sweep.C + r2;

			FVec2 s1 = _ground.GetXForm().Position + _groundAnchor1;
			FVec2 s2 = _ground.GetXForm().Position + _groundAnchor2;
			...
			Fix64 cr1u1 = FVec2.Cross(r1, _u1);
			Fix64 cr2u2 = FVec2.Cross(r2, _u2);
			...
			if (step.WarmStarting)
			{
				...
				// Warm starting.
				FVec2 P1 = -(_impulse + _limitImpulse1) * _u1;
				FVec2 P2 = (-_ratio * _impulse - _limitImpulse2) * _u2;
				b1._linearVelocity += b1._invMass * P1;
				b1._angularVelocity += b1._invI * FVec2.Cross(r1, P1);
				b2._linearVelocity += b2._invMass * P2;
				b2._angularVelocity += b2._invI * FVec2.Cross(r2, P2);
			}
			else
			{
				_impulse = Fix64.Zero;
		...
		internal override bool SolvePositionConstraints(Fix64 baumgarte)
		{
			...
			if (_state == LimitState.AtUpperLimit)
			{
				...
				b1._sweep.A += b1._invI * FVec2.Cross(r1, P1);
				b2._sweep.C += b2._invMass * P2;
				b2._sweep.A += b2._invI * FVec2.Cross(r2, P2);
