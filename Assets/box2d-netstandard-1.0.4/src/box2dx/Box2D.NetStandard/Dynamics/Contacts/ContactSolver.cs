    ...
    public class ContactSolver : IDisposable
    {
        ...
        public ContactSolver(TimeStep step, Contact[] contacts, int contactCount)
        {
            ...
            for (int i = 0; i < _constraintCount; ++i)
            {
                ...
                FVec2 vA = bodyA._linearVelocity;
                FVec2 vB = bodyB._linearVelocity;
                ...
                unsafe
                {
                    fixed (ContactConstraintPoint* ccPointsPtr = cc.Points)
                    {
                        for (int j = 0; j < cc.PointCount; ++j)
                        {
                            ...
                            Fix64 rnA = FVec2.Cross(ccp->RA, cc.Normal);
                            Fix64 rnB = FVec2.Cross(ccp->RB, cc.Normal);
                            rnA *= rnA;
                            rnB *= rnB;
                            ...
                            FVec2 tangent = FVec2.Cross(cc.Normal, Fix64.One);

                            Fix64 rtA = FVec2.Cross(ccp->RA, tangent);
                            Fix64 rtB = FVec2.Cross(ccp->RB, tangent);
                            rtA *= rtA;
                            rtB *= rtB;
                            ...
                            Fix64 vRel = FVec2.Dot(cc.Normal, vB + FVec2.Cross(wB, ccp->RB) - vA - FVec2.Cross(wA, ccp->RA));
                        ...
                        if (cc.PointCount == 2)
                        {
                            ...
                            Fix64 rn1A = FVec2.Cross(ccp1->RA, cc.Normal);
                            Fix64 rn1B = FVec2.Cross(ccp1->RB, cc.Normal);
                            Fix64 rn2A = FVec2.Cross(ccp2->RA, cc.Normal);
                            Fix64 rn2B = FVec2.Cross(ccp2->RB, cc.Normal);
                            ...
                            Fix64 k_maxConditionNumber = (Fix64)100.0f;
        ...
        public void InitVelocityConstraints(TimeStep step)
        {
            unsafe
            {
                ...
                for (int i = 0; i < _constraintCount; ++i)
                {
                    ...
                    Fix64 invMassB = bodyB._invMass;
                    Fix64 invIB = bodyB._invI;
                    FVec2 normal = c.Normal;
                    FVec2 tangent = FVec2.Cross(normal, Fix64.One);
                    ...
                    fixed (ContactConstraintPoint* pointsPtr = c.Points)
                    {
                        if (step.WarmStarting)
                        {
                            for (int j = 0; j < c.PointCount; ++j)
                            {
                                ...
                                FVec2 P = ccp->NormalImpulse * normal + ccp->TangentImpulse * tangent;
                                bodyA._angularVelocity -= invIA * FVec2.Cross(ccp->RA, P);
                                bodyA._linearVelocity -= invMassA * P;
                                bodyB._angularVelocity += invIB * FVec2.Cross(ccp->RB, P);
                ...
                unsafe
                {
                    fixed (ContactConstraintPoint* pointsPtr = c.Points)
                    {
                        ...
                        for (int j = 0; j < c.PointCount; ++j)
                        {
                            ...
                            FVec2 dv = vB + FVec2.Cross(wB, ccp->RB) - vA - FVec2.Cross(wA, ccp->RA);

                            // Compute tangent force
                            Fix64 vt = FVec2.Dot(dv, tangent);
                            ...
                            FVec2 P = lambda * tangent;

                            vA -= invMassA * P;
                            wA -= invIA * FVec2.Cross(ccp->RA, P);

                            vB += invMassB * P;
                            wB += invIB * FVec2.Cross(ccp->RB, P);
                        ...
                        else
                        {
                            ...
                            FVec2 a = new FVec2(cp1->NormalImpulse, cp2->NormalImpulse);
                            Box2DXDebug.Assert(a.X >= Fix64.Zero && a.Y >= Fix64.Zero);

                            // Relative velocity at contact
                            FVec2 dv1 = vB + FVec2.Cross(wB, cp1->RB) - vA - FVec2.Cross(wA, cp1->RA);
                            FVec2 dv2 = vB + FVec2.Cross(wB, cp2->RB) - vA - FVec2.Cross(wA, cp2->RA);

                            // Compute normal velocity
                            Fix64 vn1 = FVec2.Dot(dv1, normal);
                            Fix64 vn2 = FVec2.Dot(dv2, normal);
                            ...
                            for (; ; )
                            {
                                ...
                                // x' = - inv(A) * b'
                                //
                                FVec2 x = -Common.Math.Mul(c.NormalMass, b);
                                ...
                                if (x.X >= Fix64.Zero && x.Y >= Fix64.Zero)
                                {
                                    ...
                                    FVec2 P2 = d.Y * normal;
                                    vA -= invMassA * (P1 + P2);
                                    wA -= invIA * (FVec2.Cross(cp1->RA, P1) + FVec2.Cross(cp2->RA, P2));

                                    vB += invMassB * (P1 + P2);
                                    wB += invIB * (FVec2.Cross(cp1->RB, P1) + FVec2.Cross(cp2->RB, P2));
...
#if DEBUG_SOLVER
									// Postconditions
									dv1 = vB + FVec2.Cross(wB, cp1->RB) - vA - FVec2.Cross(wA, cp1->RA);
									dv2 = vB + FVec2.Cross(wB, cp2->RB) - vA - FVec2.Cross(wA, cp2->RA);

									// Compute normal velocity
									vn1 = FVec2.Dot(dv1, normal);
									vn2 = FVec2.Dot(dv2, normal);
        ...
        {
            internal FVec2 Normal;
            internal FVec2[] Points = new FVec2[Settings.MaxManifoldPoints];
            ...
            internal void Initialize(ContactConstraint cc)
            {
                ...
                switch (cc.Type)
                {
                    case ManifoldType.Circles:
                        {
                            FVec2 pointA = cc.BodyA.GetWorldPoint(cc.LocalPoint);
                            FVec2 pointB = cc.BodyB.GetWorldPoint(cc.Points[0].LocalPoint);
                            if (FVec2.DistanceSquared(pointA, pointB) > Settings.FLT_EPSILON_SQUARED)
                            {
                            ...
                            Points[0] = (Fix64)0.5f * (pointA + pointB);
                            Separations[0] = FVec2.Dot(pointB - pointA, Normal) - cc.Radius;
                        }
                        break;
                    ...
                    case ManifoldType.FaceA:
                        {
                            ...
                            FVec2 planePoint = cc.BodyA.GetWorldPoint(cc.LocalPoint);

                            for (int i = 0; i < cc.PointCount; ++i)
                            {
                                FVec2 clipPoint = cc.BodyB.GetWorldPoint(cc.Points[i].LocalPoint);
                                Separations[i] = FVec2.Dot(clipPoint - planePoint, Normal) - cc.Radius;
                                Points[i] = clipPoint;
                            }
                        }
                        break;
                    ...
                    case ManifoldType.FaceB:
                        {
                            ...
                            FVec2 planePoint = cc.BodyB.GetWorldPoint(cc.LocalPoint);

                            for (int i = 0; i < cc.PointCount; ++i)
                            {
                                FVec2 clipPoint = cc.BodyA.GetWorldPoint(cc.Points[i].LocalPoint);
                                Separations[i] = FVec2.Dot(clipPoint - planePoint, Normal) - cc.Radius;
                                Points[i] = clipPoint;
                            }
        ...
        public bool SolvePositionConstraints(Fix64 baumgarte)
        {
            ...
            for (int i = 0; i < _constraintCount; ++i)
            {
                ...
                FVec2 normal = s_PositionSolverManifold.Normal;

                // Solver normal constraints
                for (int j = 0; j < c.PointCount; ++j)
                {
                    FVec2 point = s_PositionSolverManifold.Points[j];
                    Fix64 separation = s_PositionSolverManifold.Separations[j];

                    FVec2 rA = point - bodyA._sweep.C;
                    FVec2 rB = point - bodyB._sweep.C;
                    ...
                    FVec2 P = impulse * normal;

                    bodyA._sweep.C -= invMassA * P;
                    bodyA._sweep.A -= invIA * FVec2.Cross(rA, P);
                    bodyA.SynchronizeTransform();

                    bodyB._sweep.C += invMassB * P;
                    bodyB._sweep.A += invIB * FVec2.Cross(rB, P);
            ...
            return minSeparation >= -(Fix64)1.5f * Settings.LinearSlop;
        }
    }
}
