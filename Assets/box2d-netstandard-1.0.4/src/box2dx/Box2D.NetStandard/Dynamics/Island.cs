...
namespace Box2DX.Dynamics
{
    ...
    public class Island : IDisposable
    {
        ...
        public Island(int bodyCapacity, int contactCapacity, int jointCapacity, ContactListener listener)
        {
            ...
            _joints = new BJoint[jointCapacity];
        ...
        public void Solve(TimeStep step, FVec2 gravity, bool allowSleep)
        {
            ...
            for (int i = 0; i < _bodyCount; ++i)
            {
                ...
                // Reset forces.
                b._force.Set(Fix64.Zero, Fix64.Zero);
                b._torque = Fix64.Zero;
                ...
                b._linearVelocity *= Common.Math.Clamp(Fix64.One - step.Dt * b._linearDamping, Fix64.Zero, Fix64.One);
                b._angularVelocity *= Common.Math.Clamp(Fix64.One - step.Dt * b._angularDamping, Fix64.Zero, Fix64.One);
            }
            ...
            for (int i = 0; i < _bodyCount; ++i)
            {
                ...
                FVec2 translation = step.Dt * b._linearVelocity;
                if (Common.FVec2.Dot(translation, translation) > Settings.MaxTranslationSquared)
                {
                    translation.Normalize();
                    b._linearVelocity = (Settings.MaxTranslation * step.Inv_Dt) * translation;
                }

                Fix64 rotation = step.Dt * b._angularVelocity;
            ...
            if (allowSleep)
            {
                Fix64 minSleepTime = Settings.FLT_MAX;

#if !TARGET_FLOAT32_IS_FIXED
                Fix64 linTolSqr = Settings.LinearSleepTolerance * Settings.LinearSleepTolerance;
                Fix64 angTolSqr = Settings.AngularSleepTolerance * Settings.AngularSleepTolerance;
#endif

                for (int i = 0; i < _bodyCount; ++i)
                {
                    Body b = _bodies[i];
                    if (b._invMass == Fix64.Zero)
                    {
                        continue;
                    }
 ...
 b._angularVelocity * b._angularVelocity > angTolSqr ||
                        FVec2.Dot(b._linearVelocity, b._linearVelocity) > linTolSqr)
#endif
                    {
                        b._sleepTime = Fix64.Zero;
                        minSleepTime = Fix64.Zero;
                ...
                if (minSleepTime >= Settings.TimeToSleep)
                {
                    for (int i = 0; i < _bodyCount; ++i)
                    {
                        ...
                        b._linearVelocity = FVec2.Zero;
                        b._angularVelocity = Fix64.Zero;
                    }
                }
            }
        }
        ...
        public void SolveTOI(ref TimeStep subStep)
        {
            ...
            for (int i = 0; i < _bodyCount; ++i)
            {
                ...
                // Check for large velocities.
                FVec2 translation = subStep.Dt * b._linearVelocity;
                if (FVec2.Dot(translation, translation) > Settings.MaxTranslationSquared)
                {
                    translation.Normalize();
                    b._linearVelocity = (Settings.MaxTranslation * subStep.Inv_Dt) * translation;
                }

                Fix64 rotation = subStep.Dt * b._angularVelocity;
                if (rotation * rotation > Settings.MaxRotationSquared)
                {
                    if (rotation < Fix64.Zero)
                    {
            ...
            Fix64 k_toiBaumgarte = (Fix64)0.75f;
