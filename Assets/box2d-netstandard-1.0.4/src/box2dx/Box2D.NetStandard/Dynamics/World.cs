...
namespace Box2DX.Dynamics
{
    ...
    {
        public Fix64 Dt; // time step
        public Fix64 Inv_Dt; // inverse time step (0 if dt == 0).
        public Fix64 DtRatio;   // dt * inv_dt0
    ...
    public class World : IDisposable
    {
        ...
        private BJoint _jointList;
        private Controllers.Controller _controllerList;

        private FVec2 _raycastNormal;
        ...
        private FVec2 _gravity;
        /// <summary>
        /// Get\Set global gravity vector.
        /// </summary>
        public FVec2 Gravity { get { return _gravity; } set { _gravity = value; } }
        ...
        public World(AABB worldAABB, FVec2 gravity, bool doSleep)
        {
        ...
        public void DestroyBody(Body b)
        {
            ...
            while (jn != null)
            {
                ...
                {
                    _destructionListener.SayGoodbye(jn0.BJoint);
                }

                DestroyBJoint(jn0.BJoint);
            }
        ...
        /// <param name="def"></param>
        /// <returns></returns>
        public BJoint CreateBJoint(BJointDef def)
        {
            Box2DXDebug.Assert(_lock == false);

            BJoint j = BJoint.Create(def);
        ...
        /// </summary>
        /// <param name="j"></param>
        public void DestroyBJoint(BJoint j)
        {
            Box2DXDebug.Assert(_lock == false);
        ...
        /// <summary>
        /// Get the world joint list. With the returned joint, use BJoint.GetNext to get
        /// the next joint in the world list. A null joint indicates the end of the list.
        /// </summary>
        /// <returns>The head of the world joint list.</returns>
        public BJoint GetBJointList()
        {
            return _jointList;
        }
        ...
        /// </summary>
        /// <returns></returns>
        public int GetBJointCount() { return _jointCount; }
        ...
        public void Step(Fix64 dt, int velocityIterations, int positionIteration)
        {
            _lock = true;
            ...
            if (dt > Fix64.Zero)
            {
                step.Inv_Dt = Fix64.One / dt;
            }
            else
            {
                step.Inv_Dt = Fix64.Zero;
            }
            ...
            // Handle TOI events.
            if (_continuousPhysics && step.Dt > Fix64.Zero)
            {
                SolveTOI(step);
            }
...
#warning "PTR"
        ...
        public Fixture RaycastOne(Segment segment, out Fix64 lambda, out FVec2 normal, bool solidShapes, object userData)
        {
            int maxCount = 1;
            Fixture[] fixture;
            lambda = Fix64.Zero;
        ...
        private void Solve(TimeStep step)
        {
            ...
            for (BJoint j = _jointList; j != null; j = j._next)
            {
                j._islandFlag = false;
            }
            ...
            int stackSize = _bodyCount;
            {
                ...
                for (Body seed = _bodyList; seed != null; seed = seed._next)
                {
                    ...
                    while (stackCount > 0)
                    {
                        ...
                        for (BJointEdge jn = b._jointList; jn != null; jn = jn.Next)
                        {
                            if (jn.BJoint._islandFlag == true)
                            {
                                continue;
                            }

                            island.Add(jn.BJoint);
                            jn.BJoint._islandFlag = true;
        ...
        private void SolveTOI(TimeStep step)
        {
            ...
            Island island = new Island(_bodyCount, Settings.MaxTOIContactsPerIsland, Settings.MaxTOIBJointsPerIsland, _contactListener);
            ...
            for (; ; )
            {
                ...
                for (Contact c = _contactList; c != null; c = c._next)
                {
                    ...
                    else
                    {
                        ...
                        Box2DXDebug.Assert(t0 < Fix64.One);
                        ...
                        Box2DXDebug.Assert(Fix64.Zero <= toi && toi <= Fix64.One);

                        // If the TOI is in range ...
                        if (Fix64.Zero < toi && toi < Fix64.One)
                        {
                            // Interpolate on the actual range.
                            toi = Common.Math.Min((Fix64.One - toi) * t0 + toi, Fix64.One);
                        }
                ...
                if (minContact == null || Fix64.One - ((Fix64)100.0f) * Settings.FLT_EPSILON < minTOI)
                {
                ...
                while (queueSize > 0)
                {
                    ...
                    for (BJointEdge jEdge = b._jointList; jEdge != null; jEdge = jEdge.Next)
                    {
                        ...
                        if (jEdge.BJoint._islandFlag == true)
                        {
                            continue;
                        }

                        island.Add(jEdge.BJoint);

                        jEdge.BJoint._islandFlag = true;
                ...
                subStep.Dt = (Fix64.One - minTOI) * step.Dt;
                subStep.Inv_Dt = Fix64.One / subStep.Dt;
                subStep.DtRatio = Fix64.Zero;
        ...
        private void DrawBJoint(BJoint joint)
        {
            Body b1 = joint.GetBody1();
            Body b2 = joint.GetBody2();
            XForm xf1 = b1.GetXForm();
            XForm xf2 = b2.GetXForm();
            FVec2 x1 = xf1.Position;
            FVec2 x2 = xf2.Position;
            FVec2 p1 = joint.Anchor1;
            FVec2 p2 = joint.Anchor2;

            Color color = new Color((Fix64)0.5f, (Fix64)0.8f, (Fix64)0.8f);
            ...
            switch (joint.GetType())
            {
                ...
                case BJointType.PulleyBJoint:
                    {
                        PulleyBJoint pulley = (PulleyBJoint)joint;
                        FVec2 s1 = pulley.GroundAnchor1;
                        FVec2 s2 = pulley.GroundAnchor2;
...
#warning "the core argument is not used, the coreColor variable is also not used"
            Color coreColor = new Color((Fix64)0.9f, (Fix64)0.6f, (Fix64)0.6f);
                ...
                case ShapeType.CircleShape:
                    {
                        ...
                        FVec2 center = Common.Math.Mul(xf, circle._position);
                        Fix64 radius = circle._radius;
                        FVec2 axis = xf.R.Col1;
                ...
                case ShapeType.PolygonShape:
                    {
                        ...
                        FVec2[] localVertices = poly._vertices;

                        Box2DXDebug.Assert(vertexCount <= Settings.MaxPolygonVertices);
                        FVec2[] vertices = new FVec2[Settings.MaxPolygonVertices];
        ...
        private void DrawDebugData()
        {
            ...
            if ((flags & DebugDraw.DrawFlags.Shape) != 0)
            {
                ...
                for (Body b = _bodyList; b != null; b = b.GetNext())
                {
                    ...
                    for (Fixture f = b.GetFixtureList(); f != null; f = f.Next)
                    {
                        if (b.IsStatic())
                        {
                            DrawFixture(f, xf, new Color((Fix64)0.5f, (Fix64)0.9f, (Fix64)0.5f), core);
                        }
                        else if (b.IsSleeping())
                        {
                            DrawFixture(f, xf, new Color((Fix64)0.5f, (Fix64)0.5f, (Fix64)0.9f), core);
                        }
                        else
                        {
                            DrawFixture(f, xf, new Color((Fix64)0.9f, (Fix64)0.9f, (Fix64)0.9f), core);
                        }
                    }
                }
            }

            if ((flags & DebugDraw.DrawFlags.BJoint) != 0)
            {
                for (BJoint j = _jointList; j != null; j = j.GetNext())
                {
                    if (j.GetType() != BJointType.MouseBJoint)
                    {
                        DrawBJoint(j);
                    }
                }
            ...
            if ((flags & DebugDraw.DrawFlags.Pair) != 0)
            {
                ...
                FVec2 invQ = new FVec2();
                invQ.Set(Fix64.One / bp._quantizationFactor.X, Fix64.One / bp._quantizationFactor.Y);
                Color color = new Color((Fix64)0.9f, (Fix64)0.9f, (Fix64)0.3f);
                ...
                for (int i = 0; i < PairManager.TableCapacity; ++i)
                {
                    ...
                    while (index != PairManager.NullPair)
                    {
                        ...
                        FVec2 x1 = (Fix64)0.5f * (b1.LowerBound + b1.UpperBound);
                        FVec2 x2 = (Fix64)0.5f * (b2.LowerBound + b2.UpperBound);
            ...
            {
                BroadPhase bp = _broadPhase;
                FVec2 worldLower = bp._worldAABB.LowerBound;
                FVec2 worldUpper = bp._worldAABB.UpperBound;

                FVec2 invQ = new FVec2();
                invQ.Set(Fix64.One / bp._quantizationFactor.X, Fix64.One / bp._quantizationFactor.Y);
                Color color = new Color((Fix64)0.9f, (Fix64)0.3f, (Fix64)0.9f);
                ...
                _debugDraw.DrawPolygon(vs, 4, new Color((Fix64)0.3f, (Fix64)0.9f, (Fix64)0.9f));
            }
        ...
        private static Fix64 RaycastSortKey(object data)
        {
