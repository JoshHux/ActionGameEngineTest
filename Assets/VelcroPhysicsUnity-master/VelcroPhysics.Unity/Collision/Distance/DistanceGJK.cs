...
namespace VelcroPhysics.Collision.Distance
{
    ...
    public static class DistanceGJK
    {
        ...
        public static void ComputeDistance(ref DistanceInput input, out DistanceOutput output, out SimplexCache cache)
        {
            ...
            simplex.ReadCache(ref cache, ref input.ProxyA, ref input.VTransformA, ref input.ProxyB,
                ref input.VTransformB);
            ...
            while (iter < Settings.MaxGJKIterations)
            {
                ...
                vertex.IndexA = input.ProxyA.GetSupport(MathUtils.MulT(input.VTransformA.q, -d));
                vertex.WA = MathUtils.Mul(ref input.VTransformA, input.ProxyA.Vertices[vertex.IndexA]);

                vertex.IndexB = input.ProxyB.GetSupport(MathUtils.MulT(input.VTransformB.q, d));
                vertex.WB = MathUtils.Mul(ref input.VTransformB, input.ProxyB.Vertices[vertex.IndexB]);
