...
namespace Spax.Input
{
    public class InputReader : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            //
            controls["Move"].performed += ctx => CallbackDirectionInput(ctx.ReadValue<Vector2>());
            controls["Move"].canceled += ctx => CallbackDirectionInput(ctx.ReadValue<Vector2>());
            
            controls["Jump"].performed += ctx => CallbackButtonInput(Button.W);
            controls["Jump"].canceled += ctx => CallbackButtonInput(Button.W);
            
            controls["LightAttack"].performed += ctx => CallbackButtonInput(Button.I);
            controls["LightAttack"].canceled += ctx => CallbackButtonInput(Button.I);
