...
namespace BEPUutilities
{
    ...
    public static partial class ConvexHullHelper
    {
        ...
        public static void GetConvexHull(IList<BepuVector3> points, IList<int> outputTriangleIndices)
        {
        ...
        public static void GetConvexHull(IList<BepuVector3> points, IList<BepuVector3> outputSurfacePoints)
        {
            var rawPoints = CommonResources.GetVectorList();
            rawPoints.AddRange(points);
        ...
        /// <param name="points">List of points in the set.</param>
        /// <param name="outputSurfacePoints">Unique points on the surface of the convex hull.</param>
        public static void GetConvexHull(RawList<BepuVector3> points, IList<BepuVector3> outputSurfacePoints)
        {
        ...
        /// Each group of 3 indices represents a triangle on the surface of the hull.</param>
        /// <param name="outputSurfacePoints">Unique points on the surface of the convex hull.</param>
        public static void GetConvexHull(IList<BepuVector3> points, IList<int> outputTriangleIndices, IList<BepuVector3> outputSurfacePoints)
        {
            var rawPoints = CommonResources.GetVectorList();
            var rawIndices = CommonResources.GetIntList();
            rawPoints.AddRange(points);
        ...
        /// Each group of 3 indices represents a triangle on the surface of the hull.</param>
        /// <param name="outputSurfacePoints">Unique points on the surface of the convex hull.</param>
        public static void GetConvexHull(RawList<BepuVector3> points, RawList<int> outputTriangleIndices, IList<BepuVector3> outputSurfacePoints)
        {
        ...
        /// <param name="outputTriangleIndices">List of indices into the input point set composing the triangulated surface of the convex hull.
        /// Each group of 3 indices represents a triangle on the surface of the hull.</param>
        public static void GetConvexHull(RawList<BepuVector3> points, RawList<int> outputTriangleIndices)
        {
            if (points.Count == 0)
            {
            ...
            while (outsidePoints.Count > 0)
            {
                ...
                for (int k = 0; k < outputTriangleIndices.Count; k += 3)
                {
                    ...
                    BepuVector3 maximum = points.Elements[maxIndex];

                    //If the point is beyond the current triangle, continue.
                    BepuVector3 offset;
                    BepuVector3.Subtract(ref maximum, ref points.Elements[outputTriangleIndices.Elements[k]], out offset);
					Fix64 dot;
        ...
        private static int GetExtremePoint(ref BepuVector3 direction, RawList<BepuVector3> points, RawList<int> outsidePoints)
        {
            ...
            {
                Fix64 dot;
                BepuVector3.Dot(ref points.Elements[outsidePoints[i]], ref direction, out dot);
                if (dot > maximumDot)
                {
                    maximumDot = dot;
                    extremeIndex = i;
        ...
        private static void GetExtremePoints(ref BepuVector3 direction, RawList<BepuVector3> points, out Fix64 maximumDot, out Fix64 minimumDot, out int maximumIndex, out int minimumIndex)
        {
            maximumIndex = 0;
            minimumIndex = 0;

            Fix64 dot;
            BepuVector3.Dot(ref points.Elements[0], ref direction, out dot);
            minimumDot = dot;
            maximumDot = dot;
            for (int i = 1; i < points.Count; ++i)
            {
                BepuVector3.Dot(ref points.Elements[i], ref direction, out dot);
                if (dot > maximumDot)
                {
                    maximumDot = dot;
                    maximumIndex = i;
        ...
        private static void ComputeInitialTetrahedron(RawList<BepuVector3> points, RawList<int> outsidePointCandidates, RawList<int> triangleIndices, out BepuVector3 centroid)
        {
            ...
            BepuVector3 ab;
            BepuVector3.Subtract(ref points.Elements[b], ref points.Elements[a], out ab);
            BepuVector3.Cross(ref ab, ref Toolbox.UpVector, out direction);
            if (direction.LengthSquared() < Toolbox.Epsilon)
                BepuVector3.Cross(ref ab, ref Toolbox.RightVector, out direction);
            ...
            Fix64 dot;
            BepuVector3.Dot(ref direction, ref points.Elements[a], out dot);
            ...
            BepuVector3 ac;
            BepuVector3.Subtract(ref points.Elements[c], ref points.Elements[a], out ac);
            BepuVector3.Cross(ref ab, ref ac, out direction);
            ...
            BepuVector3.Add(ref points.Elements[a], ref points.Elements[b], out centroid);
            BepuVector3.Add(ref centroid, ref points.Elements[c], out centroid);
            BepuVector3.Add(ref centroid, ref points.Elements[d], out centroid);
            BepuVector3.Multiply(ref centroid, F64.C0p25, out centroid);
            ...
            for (int i = 0; i < triangleIndices.Count; i += 3)
            {
                ...
                BepuVector3 cross;
                BepuVector3.Subtract(ref vB, ref vA, out ab);
                BepuVector3.Subtract(ref vC, ref vA, out ac);
                BepuVector3.Cross(ref ac, ref ab, out cross);
                BepuVector3 offset;
                BepuVector3.Subtract(ref vA, ref centroid, out offset);
                Fix64 volume;
                BepuVector3.Dot(ref offset, ref cross, out volume);
        ...
        private static void RemoveInsidePoints(RawList<BepuVector3> points, RawList<int> triangleIndices, RawList<int> outsidePoints)
        {
            ...
            for (int i = 0; i < triangleIndices.Count && insidePoints.Count > 0; i += 3)
            {
                ...
                BepuVector3 p = points.Elements[triangleIndices.Elements[i]];
                ...
                for (int j = insidePoints.Count - 1; j >= 0; --j)
                {
                    ...
                    BepuVector3 offset;
                    BepuVector3.Subtract(ref points.Elements[insidePoints.Elements[j]], ref p, out offset);
                    Fix64 dot;
                    BepuVector3.Dot(ref offset, ref normal, out dot);
        ...
        private static void FindNormal(RawList<int> indices, RawList<BepuVector3> points, int triangleIndex, out BepuVector3 normal)
        {
            var a = points.Elements[indices.Elements[triangleIndex]];
            BepuVector3 ab, ac;
            BepuVector3.Subtract(ref points.Elements[indices.Elements[triangleIndex + 1]], ref a, out ab);
            BepuVector3.Subtract(ref points.Elements[indices.Elements[triangleIndex + 2]], ref a, out ac);
            BepuVector3.Cross(ref ac, ref ab, out normal);
        }

        private static bool IsTriangleVisibleFromPoint(RawList<int> indices, RawList<BepuVector3> points, int triangleIndex, ref BepuVector3 point)
        {
            ...
            BepuVector3 offset;
            BepuVector3.Subtract(ref point, ref a, out offset);
            Fix64 dot;
            BepuVector3.Dot(ref offset, ref normal, out dot);
            return dot >= F64.C0;
        }

        private static void VerifyWindings(RawList<int> newIndices, RawList<BepuVector3> points, ref BepuVector3 centroid)
        {
