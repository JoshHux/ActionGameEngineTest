...
namespace BEPUphysics.Constraints.SolverGroups
{
    ...
    public class RevoluteJoint : SolverGroup
    {
        ...
        public RevoluteJoint(Entity connectionA, Entity connectionB, BepuVector3 anchor, BepuVector3 freeAxis)
        {
            if (connectionA == null)
            ...
            baseAxis -= BepuVector3.Dot(baseAxis, freeAxis) * freeAxis;
            if (baseAxis.LengthSquared() < Toolbox.BigEpsilon)
            {
                ...
                baseAxis = BepuVector3.Cross(freeAxis, BepuVector3.Up);
                if (baseAxis.LengthSquared() < Toolbox.BigEpsilon)
                {
                    baseAxis = BepuVector3.Cross(freeAxis, BepuVector3.Right);
                }
            }
