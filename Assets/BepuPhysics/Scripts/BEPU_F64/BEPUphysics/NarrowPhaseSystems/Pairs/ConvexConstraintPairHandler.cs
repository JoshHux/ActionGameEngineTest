...
namespace BEPUphysics.NarrowPhaseSystems.Pairs
{
    ...
    public abstract class ConvexConstraintPairHandler : ConvexPairHandler
    {
        ...
        protected internal override void GetContactInformation(int index, out ContactInformation info)
        {
            ...
            Fix64 radius;
            BepuVector3.Distance(ref contactConstraint.slidingFriction.manifoldCenter, ref info.Contact.Position, out radius);
            ...
            if (EntityA != null)
            {
                BepuVector3.Subtract(ref info.Contact.Position, ref EntityA.position, out velocity);
                BepuVector3.Cross(ref EntityA.angularVelocity, ref velocity, out velocity);
                BepuVector3.Add(ref velocity, ref EntityA.linearVelocity, out info.RelativeVelocity);
            }
            else
            ...
            if (EntityB != null)
            {
                BepuVector3.Subtract(ref info.Contact.Position, ref EntityB.position, out velocity);
                BepuVector3.Cross(ref EntityB.angularVelocity, ref velocity, out velocity);
                BepuVector3.Add(ref velocity, ref EntityB.linearVelocity, out velocity);
