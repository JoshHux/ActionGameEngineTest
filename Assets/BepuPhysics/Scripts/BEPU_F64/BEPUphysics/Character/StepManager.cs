...
namespace BEPUphysics.Character
{
    ...
    public class StepManager
    {
        ...
        Fix64 maximumStepHeight = F64.C1;
        /// <summary>
        ...
        public Fix64 MinimumDownStepHeight
        {
            ...
            set
            {
                if (minimumDownStepHeight < F64.C0)
                    throw new ArgumentException("Value must be nonnegative.");
        ...
        public StepManager(Cylinder characterBody, CharacterContactCategorizer contactCategorizer, SupportFinder supportFinder, QueryManager queryManager, HorizontalMotionConstraint horizontalMotionConstraint)
        {
            ...
            minimumUpStepHeight = CollisionDetectionSettings.AllowedPenetration * F64.C1p1;// Math.Max(0, -.01f + character.Body.CollisionInformation.Shape.CollisionMargin * (1 - character.SupportFinder.sinMaximumSlope));
        ...
        public bool TryToStepDown(out BepuVector3 newPosition)
        {
            ...
            BepuVector3 normal = SupportFinder.SupportRayData.Value.HitData.Normal;
            ...
            BepuVector3 intersection;
            Plane plane = new Plane(normal, BepuVector3.Dot(SupportFinder.SupportRayData.Value.HitData.Location, normal));
            BepuVector3 candidatePosition;
                ...
                if (Toolbox.GetRayPlaneIntersection(ref ray, ref plane, out hitT, out intersection))
                {
                                                ...
                                                out hintOffset))
                    {
                        ...
                        case CharacterContactPositionState.NoHit:
                            ...
                            currentOffset = (lowestBound + currentOffset) * F64.C0p5;
                            break;
                        case CharacterContactPositionState.Obstructed:
                            lowestBound = currentOffset;
                            currentOffset = (highestBound + currentOffset) * F64.C0p5;
                            break;
        ...
        CharacterContactPositionState TryDownStepPosition(ref BepuVector3 position, ref BepuVector3 down,
            ...
            out Fix64 hintOffset)
        {
            ...
            if (tractionContacts.Count > 0)
            {
                ...
                if (supportState == CharacterContactPositionState.Accepted)
                {
                    ...
                    hintOffset = -BepuVector3.Dot(supportContact.Contact.Normal, down) * supportContact.Contact.PenetrationDepth;
                ...
                else if (supportState == CharacterContactPositionState.TooDeep)
                {
                    ...
                    hintOffset = MathHelper.Min(F64.C0, F64.C0p001 - BepuVector3.Dot(supportContact.Contact.Normal, down) * supportContact.Contact.PenetrationDepth);
                ...
                else //if (supportState == SupportState.Separated)
                {
                    ...
                    hintOffset = -F64.C0p001 - BepuVector3.Dot(supportContact.Contact.Normal, down) * supportContact.Contact.PenetrationDepth;
        ...
        public bool TryToStepUp(out BepuVector3 newPosition)
        {
        ...
        void FindUpStepCandidates(RawList<ContactData> outputStepCandidates, ref BepuVector3 down)
        {
            BepuVector3 movementDirection = HorizontalMotionConstraint.MovementDirection3d;
            foreach (var c in SupportFinder.SideContacts)
            {
                ...
                Fix64 dot;
                BepuVector3.Dot(ref contact.Normal, ref movementDirection, out dot);
                if (dot > F64.C0)
                {
                    //It is! But is it low enough?
                    dot = BepuVector3.Dot(down, c.Contact.Position - characterBody.Position);
                    ...
                    if (dot < characterBody.Height * F64.C0p5 && dot > characterBody.Height * F64.C0p5 - maximumStepHeight - upStepMargin)
                    {
                        ...
                        {
                            BepuVector3.Dot(ref outputStepCandidates.Elements[i].Normal, ref contact.Normal, out dot);
                            if (dot > F64.C0p99)
                            {
        ...
        bool TryToStepUsingContact(ref ContactData contact, ref BepuVector3 down, out BepuVector3 newPosition)
        {
            BepuVector3 position = characterBody.Position;
            ...
            BepuVector3 normal = contact.Normal;
            Fix64 dot;
            BepuVector3.Dot(ref normal, ref down, out dot);
            BepuVector3 error;
            BepuVector3.Multiply(ref down, dot, out error);
            BepuVector3.Subtract(ref normal, ref error, out normal);
            normal.Normalize();
            ...
            BepuVector3.Multiply(ref down, characterBody.Height * F64.C0p5 - downRayLength, out ray.Position);
            BepuVector3.Add(ref ray.Position, ref position, out ray.Position);
            ...
            BepuVector3 horizontalOffset;
            BepuVector3.Multiply(ref normal, length, out horizontalOffset);
            BepuVector3.Add(ref ray.Position, ref horizontalOffset, out ray.Position);
            ray.Direction = down;

            //Find the earliest hit, if any.
            RayHit earliestHit;
            if (!QueryManager.RayCast(ray, downRayLength, out earliestHit) || //Can't do anything if it didn't hit.
                earliestHit.T <= F64.C0 || //Can't do anything if the hit was invalid.
            ...
            BepuVector3 supportNormal;
            BepuVector3.Normalize(ref earliestHit.Normal, out supportNormal);
            //Calibrate the normal to face in the same direction as the down vector for consistency.
            BepuVector3.Dot(ref supportNormal, ref down, out dot);
            if (dot < F64.C0)
            {
                BepuVector3.Negate(ref supportNormal, out supportNormal);
                dot = -dot;
            }
            ...
            BepuVector3.Negate(ref down, out ray.Direction);
            //Find the earliest hit, if any.
            ...
            BepuVector3.Multiply(ref normal, horizontalOffsetAmount, out horizontalOffset);
            BepuVector3.Add(ref transform.Position, ref horizontalOffset, out transform.Position);
            BepuVector3 verticalOffset;
            BepuVector3.Multiply(ref down, -downRayLength, out verticalOffset);
            BepuVector3.Add(ref transform.Position, ref verticalOffset, out transform.Position);
            ...
            //Intersect the ray against the plane defined by the support hit.
            BepuVector3 intersection;
            BepuVector3.Dot(ref earliestHit.Location, ref supportNormal, out dot);
                ...
                if (Toolbox.GetRayPlaneIntersection(ref downRay, ref plane, out hitT, out intersection))
                {
                                              ...
                                              out hintOffset))
                    {
                        case CharacterContactPositionState.Accepted:
                            currentOffset += hintOffset;
                            //Only use the new position location if the movement distance was the right size.
                            if (currentOffset < F64.C0 && currentOffset > -maximumStepHeight - CollisionDetectionSettings.AllowedPenetration)
                            {
        ...
        private void PrepareQueryObject(ref BepuVector3 position)
        {
        ...
        CharacterContactPositionState TryUpStepPosition(ref BepuVector3 sideNormal, ref BepuVector3 position, ref BepuVector3 down,
            ref QuickList<CharacterContact> tractionContacts, ref QuickList<CharacterContact> supportContacts, ref QuickList<CharacterContact> sideContacts, ref QuickList<CharacterContact> headContacts,
            out Fix64 hintOffset)
        {
            hintOffset = F64.C0;
            ...
            if (headContacts.Count > 0)
            {
                ...
                Fix64 dot;
                BepuVector3.Dot(ref down, ref headContacts.Elements[0].Contact.Normal, out dot);
                hintOffset = -dot * headContacts.Elements[0].Contact.PenetrationDepth;
                for (int i = 1; i < headContacts.Count; i++)
                {
                    BepuVector3.Dot(ref down, ref headContacts.Elements[i].Contact.Normal, out dot);
            ...
            if (!obstructed && supportContacts.Count > 0)
            {
                ...
                if (supportState == CharacterContactPositionState.Accepted)
                {
                    if (tractionContacts.Count > 0)
                    {
                        ...
                        hintOffset = MathHelper.Min(F64.C0, BepuVector3.Dot(supportContact.Contact.Normal, down) * (CollisionDetectionSettings.AllowedPenetration * F64.C0p5 - supportContact.Contact.PenetrationDepth));
                        return CharacterContactPositionState.Accepted;
                    }
                    else
                    {
                        ...
                        Ray downRay;
                        downRay.Position = supportContact.Contact.Position + sideNormal * F64.C0p001;
                        Fix64 verticalOffset = BepuVector3.Dot(downRay.Position - position, down);
                        verticalOffset = characterBody.Height * F64.C0p5 + verticalOffset;
                        ...
                        obstructionTestRay.Position = position - down * (characterBody.Height * F64.C0p5);
                        obstructionTestRay.Direction = downRay.Position - obstructionTestRay.Position;

                        if (!QueryManager.RayCastHitAnything(obstructionTestRay, F64.C1))
                        {
                            ...
                            if (QueryManager.RayCast(downRay, characterBody.Height, out hit))
                            {
                                ...
                                if (characterBody.Height - maximumStepHeight < hit.T)
                                {
                                    ...
                                    BepuVector3.Dot(ref hit.Normal, ref down, out dot);
                                    if (Fix64.Abs(dot) > ContactCategorizer.TractionThreshold)
                                    {
                                        ...
                                        if (QueryManager.RayCast(downRay, characterBody.Height * F64.C0p5 + maximumStepHeight, out hit))
                                        {
                                            //It hit.. almost there!
                                            hit.Normal.Normalize();
                                            BepuVector3.Dot(ref hit.Normal, ref down, out dot);
        ...
        bool IsUpStepObstructedBySideContacts(ref BepuVector3 sideNormal, ref QuickList<CharacterContact> sideContacts)
        {
            //A contact is considered obstructive if its projected depth is deeper than any existing contact along the existing contacts' normals.
            for (int i = 0; i < sideContacts.Count; i++)
            {
        ...
        bool IsObstructiveToUpStepping(ref BepuVector3 sideNormal, ref ContactData contact)
        {
            ...
            Fix64 dot;
            BepuVector3.Dot(ref contact.Normal, ref sideNormal, out dot);
            ...
            foreach (var c in SupportFinder.SideContacts)
            {
                dot = BepuVector3.Dot(contact.Normal, c.Contact.Normal);
