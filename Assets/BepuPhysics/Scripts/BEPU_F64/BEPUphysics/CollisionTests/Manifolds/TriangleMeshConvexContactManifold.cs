...
namespace BEPUphysics.CollisionTests.Manifolds
{
    ...
    public abstract class TriangleMeshConvexContactManifold : ContactManifold
    {
        ...
        public override void Update(Fix64 dt)
        {
            ...
            Matrix3x3.CreateFromBepuQuaternion(ref convex.worldTransform.Orientation, out orientation);
            ...
            if (UseImprovedBoundaryHandling)
            {
                ...
                if (boundarySets.VertexContacts.Count == 0 && guaranteedContacts == 0 && boundarySets.EdgeContacts.Count > 1)
                {
                    ...
                    Fix64 dot;
                    BepuVector3.Dot(ref firstNormal, ref boundarySets.EdgeContacts.Elements[0].CorrectedNormal, out dot);
                    ...
                    else
                    {
                        ...
                        {
                            BepuVector3.Dot(ref boundarySets.EdgeContacts.Elements[i].ContactData.Normal, ref firstNormal, out dot);
                            if (dot < F64.C0)
                            {
                                atLeastOneNormalAgainst = true;
                            }
                            //Check to see if the normal is outside the plane.
                            BepuVector3.Dot(ref boundarySets.EdgeContacts.Elements[i].ContactData.Normal, ref boundarySets.EdgeContacts.Elements[0].CorrectedNormal, out dot);
                    ...
                    if (allNormalsInSamePlane && atLeastOneNormalAgainst)
                    {
                        ...
                        for (int i = 1; i < boundarySets.EdgeContacts.Count; i++)
                        {
                            ...
                            BepuVector3.Dot(ref boundarySets.EdgeContacts.Elements[i].CorrectedNormal, ref boundarySets.EdgeContacts.Elements[i].ContactData.Normal, out dot);
                            if (dot < F64.C0p01)
                            {
                ...
                for (int i = 0; i < boundarySets.VertexContacts.Count; i++)
                {
                    ...
                    else if (boundarySets.VertexContacts.Elements[i].ShouldCorrect || guaranteedContacts == 0)
                    {
                        ...
                        BepuVector3.Dot(ref boundarySets.VertexContacts.Elements[i].CorrectedNormal, ref boundarySets.VertexContacts.Elements[i].ContactData.Normal, out dot);
        ...
        void AddLocalContact(ref ContactData contact, ref Matrix3x3 orientation, ref QuickList<ContactData> candidatesToAdd)
        {
            ...
            BepuVector3.Add(ref contact.Position, ref convex.worldTransform.Position, out contact.Position);
        ...
        protected void GetNormal(ref BepuVector3 uncorrectedNormal, TriangleShape localTriangleShape, out BepuVector3 normal)
        {
            ...
            BepuVector3 AB, AC;
            BepuVector3.Subtract(ref localTriangleShape.vB, ref localTriangleShape.vA, out AB);
            BepuVector3.Subtract(ref localTriangleShape.vC, ref localTriangleShape.vA, out AC);
            ...
            switch (localTriangleShape.sidedness)
            {
                case TriangleSidedness.DoubleSided:
                    ...
                    BepuVector3.Dot(ref normal, ref uncorrectedNormal, out dot);
                    if (dot < F64.C0)
                        BepuVector3.Negate(ref normal, out normal);
                    break;
        ...
        private bool IsContactUnique(ref ContactData contactCandidate, ref QuickList<ContactData> candidatesToAdd)
        {
            ...
            {
                BepuVector3.DistanceSquared(ref contacts.Elements[i].Position, ref contactCandidate.Position, out distanceSquared);
                if (distanceSquared < CollisionDetectionSettings.ContactMinimumSeparationDistanceSquared)
                {
                    ...
                    BepuVector3.Dot(ref contacts.Elements[i].Normal, ref contactCandidate.Normal, out distanceSquared);
            ...
            for (int i = 0; i < candidatesToAdd.Count; i++)
            {
                BepuVector3.DistanceSquared(ref candidatesToAdd.Elements[i].Position, ref contactCandidate.Position, out distanceSquared);
                if (distanceSquared < CollisionDetectionSettings.ContactMinimumSeparationDistanceSquared)
                {
                    //This is a nonconvex manifold.  There will be times where a an object will be shoved into a corner such that
                    //a single position will have two reasonable normals.  If the normals aren't mostly aligned, they should NOT be considered equivalent.
                    BepuVector3.Dot(ref candidatesToAdd.Elements[i].Normal, ref contactCandidate.Normal, out distanceSquared);
                    if (Fix64.Abs(distanceSquared) >= CollisionDetectionSettings.nonconvexNormalDotMinimum)
                        return false;
                }
            }
            ...
            //{
            //    BepuVector3.DistanceSquared(ref edgeContacts.Elements[i].ContactData.Position, ref contactCandidate.Position, out distanceSquared);
            //    if (distanceSquared < CollisionDetectionSettings.ContactMinimumSeparationDistanceSquared)
            //    {
            //        return false;
            //    }
            ...
            //{
            //    BepuVector3.DistanceSquared(ref vertexContacts.Elements[i].ContactData.Position, ref contactCandidate.Position, out distanceSquared);
            //    if (distanceSquared < CollisionDetectionSettings.ContactMinimumSeparationDistanceSquared)
            //    {
            //        return false;
            //    }
        ...
        struct EdgeContact
        {
            ...
            public BepuVector3 CorrectedNormal;
            public Edge Edge;
