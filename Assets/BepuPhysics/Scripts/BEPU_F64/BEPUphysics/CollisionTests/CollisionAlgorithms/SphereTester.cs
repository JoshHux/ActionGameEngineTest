...
namespace BEPUphysics.CollisionTests.CollisionAlgorithms
{
    ...
    public static class SphereTester
    {
        ...
        public static bool AreSpheresColliding(SphereShape a, SphereShape b, ref BepuVector3 positionA, ref BepuVector3 positionB, out ContactData contact)
        {
            contact = new ContactData();

            Fix64 radiusSum = a.collisionMargin + b.collisionMargin;
            BepuVector3 centerDifference;
            BepuVector3.Subtract(ref positionB, ref positionA, out centerDifference);
            ...
            if (centerDistance < (radiusSum + CollisionDetectionSettings.maximumContactDistance) * (radiusSum + CollisionDetectionSettings.maximumContactDistance))
            {
                ...
                if (radiusSum > Toolbox.Epsilon) //This would be weird, but it is still possible to cause a NaN.
                    BepuVector3.Multiply(ref centerDifference, a.collisionMargin / (radiusSum), out  contact.Position);
                else contact.Position = new BepuVector3();
                BepuVector3.Add(ref contact.Position, ref positionA, out contact.Position);

                centerDistance = Fix64.Sqrt(centerDistance);
                if (centerDistance > Toolbox.BigEpsilon)
                {
                    BepuVector3.Divide(ref centerDifference, centerDistance, out contact.Normal);
                }
                else
                {
