    ...
    public class ContactRefresher
    {
        ...
        public static void ContactRefresh(RawList<Contact> contacts, RawValueList<ContactSupplementData> supplementData, ref RigidTransform transformA, ref RigidTransform transformB, RawList<int> toRemove)
        {
            ...
            for (int k = 0; k < contacts.Count; k++)
            {
                ...
                BepuVector3 ab;
                BepuVector3.Subtract(ref newPosB, ref newPosA, out ab);
                Fix64 dot;
                BepuVector3.Dot(ref ab, ref contacts.Elements[k].Normal, out dot);
                BepuVector3 temp;
                BepuVector3.Multiply(ref contacts.Elements[k].Normal, dot, out temp);
                BepuVector3.Subtract(ref ab, ref temp, out temp);
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        BepuVector3 newPos;
                        BepuVector3.Add(ref newPosB, ref newPosA, out newPos);
                        BepuVector3.Multiply(ref newPos, F64.C0p5, out newPos);
