...
namespace BEPUphysics.CollisionShapes.ConvexShapes
{
    ...
    public struct ConvexShapeEntry
    {
        ...
        public ConvexShapeEntry(BepuVector3 position, ConvexShape shape)
        {
        ...
        /// <param name="shape">Shape of the entry.</param>
        public ConvexShapeEntry(BepuQuaternion orientation, ConvexShape shape)
        {
    ...
    public class WrappedShape : ConvexShape
    {
        ...
        public WrappedShape(ConvexShapeEntry firstShape, ConvexShapeEntry secondShape, out BepuVector3 center)
        {
            shapes.Add(firstShape);
            shapes.Add(secondShape);
        ...
        /// <param name="center">Center of the shape before recentering.</param>
        ///<exception cref="Exception">Thrown when the shape list is empty.</exception>
        public WrappedShape(IList<ConvexShapeEntry> shapeEntries, out BepuVector3 center)
        {
            if (shapeEntries.Count == 0)
                throw new ArgumentException("Cannot create a wrapped shape with no contained shapes.");
            for (int i = 0; i < shapeEntries.Count; i++)
        ...
        public void UpdateConvexShapeInfo(out BepuVector3 center)
        {
        ...
        public override void GetLocalExtremePointWithoutMargin(ref BepuVector3 direction, out BepuVector3 extremePoint)
        {
            ...
            Fix64 maxDot;
            BepuVector3.Dot(ref extremePoint, ref direction, out maxDot);
            for (int i = 1; i < shapes.WrappedList.Count; i++)
            {
                Fix64 dot;
                BepuVector3 temp;
                ...
                BepuVector3.Dot(ref direction, ref temp, out dot);
                if (dot > maxDot)
                {
                    extremePoint = temp;
                    maxDot = dot;
